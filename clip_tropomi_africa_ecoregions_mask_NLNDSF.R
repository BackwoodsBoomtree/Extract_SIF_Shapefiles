...
tmpdir          <- "/mnt/c/Rwork"
out_dir         <- "/mnt/g/TROPOMI/esa/extracted/Africa/ecoregions/masked"
out_name        <- "/Africa_TROPOSIF_L2B_all_"
...
clip_TROPOSIF <- function(input_file, roi_file, forest_mask, out_dir, out_name, land_cover,
  ...
  for (e in 1:length(eco_names)) {
    ...
    if (nrow(crds(var_roi, df = TRUE)) == 0) {
      ...
      filename <- paste0(out_name, eco_region, "_", t, ".nc")
      out_f    <- paste0(eco_dir, filename)
...
mclapply(f_list, clip_TROPOSIF, mc.cores = 4, mc.preschedule = FALSE, roi_file = roi_file, forest_mask = forest_mask,
