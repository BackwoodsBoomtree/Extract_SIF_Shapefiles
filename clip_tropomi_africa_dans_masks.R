...
tmpdir          <- "/mnt/c/Rwork"
out_dir         <- "/mnt/g/TROPOMI/esa/extracted/Africa/Dans_masks"
out_name        <- "/Africa_TROPOSIF_L2B_"
...
notes           <- "This data has been filtered using Dan Wanyama's masks."
...
roi   <- vect(roi_file)
# roi   <- aggregate(roi)
# roi   <- project(roi, "+proj=longlat +datum=WGS84")

# Create dirs
poly_name  <- substr(basename(roi_file), 1, nchar(basename(roi_file)) - 4)
out_name   <- paste0(out_name, poly_name)
out_dir    <- paste0(out_dir, "/", poly_name)
...
clip_TROPOSIF <- function(input_file, roi, out_dir, out_name, land_cover,
  ...
  if (nrow(crds(var_roi, df = TRUE)) == 0) {
    ...
    out_f <- paste0(out_dir, out_name, "_", t, ".nc")
    
    if (!is.null(land_cover_perc)) {
      ncout <- nc_create(out_f,
                         list(time_def, lon_def, lat_def,
...
mclapply(f_list, clip_TROPOSIF, mc.cores = 5, mc.preschedule = FALSE, roi = roi,
