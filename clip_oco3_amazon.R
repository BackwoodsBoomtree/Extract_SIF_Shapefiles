...
tmpdir          <- "/mnt/c/Rwork"
out_dir         <- "/mnt/g/OCO3/extracted/amazon"
out_name        <- "/Amazon_OCO3_L2B10_"
f_list          <- list.files("/mnt/g/OCO3/B10", pattern = "*.nc", full.names = TRUE, recursive = TRUE)
...
clip_nc <- function(input_file, roi_file, out_dir, out_name, land_cover,
  ...
  if (nrow(crds(var_roi, df = TRUE)) == 0) {
    ...
    elemdim <- ncdim_def("sounding_dim", "", seq(1, nrow(df)))
